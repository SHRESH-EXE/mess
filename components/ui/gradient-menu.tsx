...
export default function GradientMenu({ items = defaultMenuItems, className = '', containerClassName = '' }: GradientMenuProps) {
  const menuItems = items;
  return (
    <div className={`flex justify-center items-center min-h-screen bg-slate-900 ${containerClassName}`}>
      <ul className={`flex gap-6 flex-wrap justify-center ${className}`}>
        {menuItems.map(({ title, icon, gradientFrom, gradientTo, onClick }, idx) => (
          <li
            key={idx}
            onClick={onClick}
