  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="rounded-3xl p-5 sm:p-7 relative overflow-hidden bg-gradient-to-br from-white/90 via-orange-50/70 to-amber-50/60 backdrop-blur-xl border border-white/90 shadow-xl shadow-orange-500/5">
        ...
        {/* Live Stalls Rush Strip */}
        <div className="mt-5 pt-4 border-t border-orange-200/60 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
          {foodCourtStalls.map(stall => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => {
                  setSelectedStallId(isSelected ? 'all' : stall.id);
                  soundEffects.playTap();
                }}
                className={`p-2.5 rounded-2xl text-left transition-all border cursor-pointer relative overflow-hidden ${
                  isSelected
                    ? 'bg-white shadow-md border-orange-400 ring-2 ring-orange-400/30 scale-[1.02]'
                    : 'bg-white/60 hover:bg-white/90 border-orange-200/60'
                }`}
              >
                <div className="flex items-center justify-between gap-1 mb-1">
                  <span className="p-1 rounded-lg bg-orange-100/70 text-orange-700">
                    {getStallIcon(stall.id, 'w-4 h-4')}
                  </span>
                  <span
                ...
                <div className="text-[10px] text-slate-500 truncate font-mono mt-0.5">
                  Queue: {stall.activeQueueCount} • ~{stall.estimatedWaitMins}m
                </div>
              </button>
            );
          })}
      ...
      {activeTabSubView === 'menu' && (
        <div className="space-y-5">
          {/* Search, Stall & Category Filter Bar */}
          <div className="bg-white/80 backdrop-blur-xl p-4 rounded-3xl border border-white/90 shadow-sm space-y-3">
            <div className="flex flex-col sm:flex-row items-center gap-3">
              ...
              <div className="relative flex-1 w-full">
                ...
                <input
                  ...
                  placeholder="Search rolls, dosas, pizzas, chai, burgers, shakes..."
                  className="w-full pl-9 pr-4 py-2.5 rounded-2xl bg-white/90 border border-orange-200/80 text-xs text-slate-800 placeholder:text-slate-400 focus:outline-hidden focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 shadow-2xs"
                />
                {searchQuery && (
                  <button
                    type="button"
              ...
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-full text-xs font-bold border transition-all cursor-pointer shrink-0 ${
                  onlyVeg
            ...
            <div className="flex items-center space-x-2 overflow-x-auto pb-1 scrollbar-none">
              ...
              >
                All Stalls ({foodCourtMenuItems.length})
              </button>
              ...
              {foodCourtStalls.map(stall => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center space-x-1.5 px-3.5 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer whitespace-nowrap shrink-0 ${
                      isSelected
                        ? 'bg-gradient-to-r from-[#ff7a30] to-[#ff9248] text-white shadow-xs'
                        : 'bg-white/70 text-slate-600 hover:bg-white border border-orange-200/60'
                    }`}
                  >
                    <span className="p-0.5 text-current">
                      {getStallIcon(stall.id, 'w-3.5 h-3.5')}
                    </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredItems.map(item => {
              ...
              return (
                <div
                  key={item.id}
                  className="rounded-3xl p-4 sm:p-5 bg-white/85 backdrop-blur-xl border border-white/90 shadow-sm hover:shadow-md transition-all flex flex-col justify-between space-y-3 group"
                >
                  <div className="space-y-2">
                    {/* Stall Name & Pure Veg indicator */}
                    <div className="flex items-center justify-between gap-1">
                      <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-orange-100/80 text-orange-800 font-mono">
                        {item.stallName}
                      </span>
                      <span className="w-3.5 h-3.5 rounded-full border-2 border-emerald-600 flex items-center justify-center p-0.5 shrink-0">
                    ...
                    {/* Allergen Warning if matches profile */}
                    {matchingAllergen && (
                      <div className="p-2 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-800 text-[11px] font-bold flex items-center space-x-1.5">
                        ...
                        <span>Contains {matchingAllergen} (matches your allergy profile)</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="pt-2 border-t border-orange-100 flex items-center justify-between gap-2">
                    ...
                    <ChromeButton
                      ...
                      className="px-4 py-2 rounded-full bg-gradient-to-r from-[#ff7a30] to-[#ff9248] hover:from-[#ea671e] hover:to-[#ff8130] text-white text-xs font-bold shadow-md shadow-orange-500/20 active:scale-95 transition-all flex items-center space-x-1.5 cursor-pointer"
                    >
